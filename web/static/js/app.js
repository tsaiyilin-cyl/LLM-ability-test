...
const App = {
    ...
    // 重新测试单个结果
    async retestResult(cardId, evalCaseId) {
        // 找到原始测试结果
        const originalResult = this.state.testResults.find(r => {
            const evalId = r.test_lang ? `${r.case_id}-${r.test_lang}` : r.case_id;
            return evalId === evalCaseId;
        });
        
        if (!originalResult) {
            Components.toast('❌ 找不到原始测试结果', 'error');
            return;
        }
        ...
        const container = document.getElementById('resultsContainer');
        if (!card || !container) {
            Components.toast('❌ 找不到结果容器', 'error');
            return;
        }
        
        // 创建加载指示器
        const loadingId = `retest-${evalCaseId}`;
        const loading = Components.createLoading(loadingId, `正在重新测试 ${originalResult.case_id}...`);
        
        // 在旧卡片前插入加载指示器
        card.parentNode.insertBefore(loading, card);
        
        // 禁用重新测试按钮
        const retestBtn = card.querySelector('.retest-result-btn');
        if (retestBtn) {
            retestBtn.disabled = true;
            retestBtn.style.opacity = '0.5';
        }
        
        try {
            // 重新运行测试
            const result = await API.runTest({
                dimension: originalResult.dimension || this.state.currentDimension,
                model: originalResult.model || model, // 使用原始模型或当前选择的模型
            ...
            if (result.success) {
                // 添加必要的字段
                result.dimension = originalResult.dimension || this.state.currentDimension;
                result.test_lang = originalResult.test_lang || 'zh';
                if (originalResult.case_id_display) {
                    result.case_id_display = originalResult.case_id_display;
                }
                
                // 注意：重新测试时不清除评估数据，用户可以保留评估结果
                // 如果用户想重新评估，可以手动删除评估数据或重新评估
                
                // 从状态中移除旧结果
                this.state.testResults = this.state.testResults.filter(r => {
                    const evalId = r.test_lang ? `${r.case_id}-${r.test_lang}` : r.case_id;
                    return evalId !== evalCaseId;
                });
                
                // 添加新结果（注意：不保留 result.evaluation，因为这是运行状态，不是评估状态）
                // 评估状态只从 localStorage 中读取
                delete result.evaluation;
                this.state.testResults.push(result);
                
                // 创建新卡片
                const newCard = Components.createResultCard(result);
                ...
                Components.toast('✅ 重新测试完成', 'success');
            } else {
                // 移除加载指示器
                document.getElementById(`loading-${loadingId}`)?.remove();
                ...
                if (retestBtn) {
                    ...
                    retestBtn.style.opacity = '1';
                }
                
                const errorCard = Components.createErrorCard(loadingId, result.error);
                card.parentNode.insertBefore(errorCard, card);
                Components.toast('❌ 重新测试失败: ' + result.error, 'error');
            }
        } catch (error) {
            // 移除加载指示器
            document.getElementById(`loading-${loadingId}`)?.remove();
            ...
            const errorCard = Components.createErrorCard(loadingId, error.message);
            card.parentNode.insertBefore(errorCard, card);
            Components.toast('❌ 重新测试失败: ' + error.message, 'error');
        }
    },
    
    // 删除单个测试结果
    deleteResult(cardId, evalCaseId) {
        ...
        Components.toast('✅ 测试结果已删除', 'success');
    },
